...
static int readArgv(FILE *fp, const char *argv0, int *Argc, char ***Argv)
{
	...
	if (POWEROF2(argc))
	{
	    /* allocate enough space for any argc up to (but not including)
	       the next power of 2 */
	    char **newargv = (char**)realloc(argv, ((2*argc-1)+1) * sizeof(*argv));
	    if (newargv == NULL)
		goto error;
	    argv = newargv;
	}
	if ((argv[argc-1] = (char*)malloc(strlen(buf)+1)) == NULL)
	    goto error;
...
static int cmdline(FILE *fp, int execute)
{
	...
	*ViewState = temp;
    ...
    (void)processCmdLine(argc, argv, &files);
    while (*files != NULL)
	    ViewState->sceneGroup->addChild (pfdLoadFile(*files++));
...
static int tetherview(FILE *fp, int execute)
{
    ...
    coord.xyz.set (0.0f, 0.0f, 0.0f);
    coord.hpr.set (0.0f, 0.0f, 0.0f);
    ...
    ViewState->tetherViews[ViewState->numTetherViews].makeCoord(&coord);
...
static int vehicle(FILE *fp, int execute)
{
    ...
    /* initialize so that trailing values may be omitted */
    thisSCScoord.xyz.set( 0.0f, 0.0f, 0.0f );
    thisSCScoord.hpr.set( 0.0f, 0.0f, 0.0f );
    thisSCSscale = 1.0f;
    ...
    thisSCSmatrix.makeCoord(&thisSCScoord);
    thisSCSmatrix.preScale(thisSCSscale, thisSCSscale, thisSCSscale, 
		  thisSCSmatrix);

    thisDCS = new pfDCS();
    thisSCS = new pfSCS(thisSCSmatrix);
    ViewState->sceneGroup->addChild (thisDCS);
    thisDCS->addChild (thisSCS);
    thisSCS->addChild (thisVehicle);

    thisDCS->setTravFuncs(PFTRAV_APP, vehicleDCSAppFunc, NULL);
    thisDCS->setTravData(PFTRAV_APP, thisPathAndTime);
