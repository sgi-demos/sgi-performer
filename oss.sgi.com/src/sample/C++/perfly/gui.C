...
initGUI(pfChannel *masterChan)
{
    ...
    masterChan->getViewport(&a, &b, &c, &d);
    
    if (ViewState->guiFormat == GUI_VERTICAL)
    {
	pfuGUIViewport(a, a + 0.22 * (b-a), c, d);
	masterChan->setViewport(a + 0.22 * (b-a), b, c, d);
    } 
    else 
    {
	pfuGUIViewport(a, b, c, c + 0.22 * (d-c));
	masterChan->setViewport(a , b, c + 0.22 * (d-c), d);
    }
...
updateGUI(int guiOn)
{
	...
	ViewState->masterChan->getViewport(&ca,&cb,&cc,&cd);
	
	if(ViewState->guiFormat == GUI_VERTICAL)
	    ViewState->masterChan->setViewport(r, cb, cc, cd);
	else
	    ViewState->masterChan->setViewport(ca, cb, t, cd);
    }
    else    /* turn off GUI */
    {
	pfuEnableGUI(FALSE);
	...
	if (Multipipe || (ViewState->numChans == 1)) 
	    ViewState->masterChan->setViewport(0.0f, 1.0f, 0.0f, 1.0f);
	...
	else
	{
	    ...
	    ca = l;
	    cc = b;
	    ViewState->masterChan->setViewport(ca, cb, cc, cd);
	}
    }
}
...
controlPanel(pfuWidget *w)
{
	...
	ViewState->masterChan->setNearFar(ViewState->near, ViewState->far);
	break;
    ...
    case GUI_FOV:  
	{
		...
		ViewState->masterChan->setFOV(ViewState->fov[0],
			   ViewState->fov[1]);
	    ...
	    {
		ViewState->masterChan->setFOV(fov, -1.0f);
		ViewState->fov[0] = fov;
		ViewState->fov[1] = -1.0;
		ViewState->fov[2] = 0.0;
	    }
	    ...
	    {
		if (InitFOV) {
		    hpr.set((((ViewState->numChans - 1)*0.5f) - c) *
			      ViewState->fov[2],
			      0.0f, 0.0f);
		    pfNotify(PFNFY_DEBUG, PFNFY_PRINT,
		...
		ViewState->chan[ChanOrder[c]]->setViewOffsets(xyz, hpr);
	    }
	}
	break;
    ...
    case GUI_AA:
	{
		...
		{
		    ViewState->fade = 0;
		    ViewState->masterChan->setLODAttr(PFLOD_FADE, 0.0f);
	    ...
	    {
		ViewState->fade = 1;
		ViewState->masterChan->setLODAttr(PFLOD_FADE, ViewState->fadeRange);
	...
	{
	    if (ViewState->fade)
		ViewState->masterChan->setLODAttr(PFLOD_FADE, 
    ...
    case GUI_PIX_LIMIT:
	ViewState->masterChan->setLODAttr(PFLOD_STRESS_PIX_LIMIT,
...
/*     ViewState->num_widgets++; */
	...
	/* calligraphic draw time in usec */
	wid = pfuNewWidget(panel, PFUGUI_SLIDER, GUI_DRAW_TIME);
	pfuWidgetDim(wid, x, y, xSize, ySize);
	pfuWidgetLabel(wid, "Draw time in usec");
	pfuWidgetActionFunc(wid, controlPanel);
...
/* 	    fprintf(stderr, "Widget %i - %i %i\n", i, x, y); */
    ...
    pfNotify(PFNFY_DEBUG,PFNFY_PRINT, "Panel %f %f %f %f - %i %i\n", xo, yo, xs, ys, x, y);
    pfNotify(PFNFY_DEBUG,PFNFY_PRINT, "tmp_total_y %i, total_y %i\n", tmp_total_y, total_y);

    /* recurse to get horizontal GUI right */
    if ((ViewState->guiFormat == GUI_HORIZONTAL) &&
	(tmp_total_y != total_y)) {
	pfNotify(PFNFY_DEBUG,PFNFY_PRINT,
		"Recursing updatePanel to get horizontal GUI right.\n");
	total_y = tmp_total_y;
	updatePanel();
    }
    else
