...
static int cmdline(FILE *fp, int execute)
{
	...
	*ViewState = temp;
    ...
    nfiles = processCmdLine(argc, argv, &files);
    while (*files != NULL)
...
#ifdef DEFUNCT
		/* if comment, discard to end of line */
		if (strprefix(str, "#")	/* shell-style comment */
		 || strprefix(str, "//") /* C++-style comment */
		 || strprefix(str, ";") /* LISP-style comment */
		 || streq(str, "rem") /* BASIC-style comment */
		 || streq(str, "comment")) /* ALGOL-style comment */
		{
