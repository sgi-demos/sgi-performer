...
#if 1 /* CUSTOM */
	"    -5 <texload_msecs>            -Set per-pipe total DTR texload time\n"
#endif /* CUSTOM */
...
#if 0 /* PERFLY */
    "o:O:Nn:p:P:q:Q:r:R:s:S:t:T:uUv:V:W:w:x:X:y:Y:z:Z23471:?";
#endif /* PERFLY */
#if 1 /* CUSTOM */
    "o:O:Nn:p:P:q:Q:r:R:s:S:t:T:uUv:V:W:w:x:X:y:Y:z:Z23475:8:91:?";
#endif /* CUSTOM */
...
processCmdLine(int argc, char *argv[], char ***files)
{
...
#if 1 /* CUSTOM */
	    /* Set total pipe DTR time */
	case '5':
	    ViewState->clip_totalDTRTime = atof(optarg);
	    ViewState->clip_totalDTRTime_dirty = 1;
	    ViewState->clip_anyParams_dirty = 1;

	    /* so we don't query the existing value from the pipe,
	       and we know to turn off the MP clip textures'
	       individual DTR times as they are added to the pipe... */
	    ViewState->clip_totalDTRTimeWasSpecified = 1;

	    break;

	    /* Set pfuXformer default to Spheric */
	case '8':
	    ViewState->xformerModel = PFITDF_SPHERIC;
	    pfNotify(PFNFY_NOTICE, PFNFY_PRINT,
		     "Changing initial motion model to pfiSpheric");
	    strcpy(ViewState->sphericPathFileName, optarg);
	    break;

	    /* Set pfuXformer default to Trackball */
	case '9':
	    ViewState->xformerModel = PFITDF_TRACKBALL;
	    pfNotify(PFNFY_NOTICE, PFNFY_PRINT,
		     "Changing initial motion model to pfiTrackball");
	    break;
#endif /* CUSTOM */
	case '2':
    ...
    *files = &(argv[optind]);
    ...
    /*
     * This seems to be one of the only opportunities
     * to allocate extra secret share memory before pfConfig() is called,
     * so do it here...
     */
    {
	extern int *_pfsortqueue_depth_first;
	_pfsortqueue_depth_first = pfMalloc(sizeof(int), pfGetSharedArena());
	*_pfsortqueue_depth_first = 0; /* breadth first by default */
    }
