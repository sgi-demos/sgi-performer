...
#endif /* PERFLY */
#if 1 /* CUSTOM */
    fprintf(stderr, "  m      Toggle terrain morphing color mode\n");
    fprintf(stderr, "  M      Toggle terrain auto-fly mode\n");
#endif /* CUSTOM */
...
toggleColor(void)
{
    ViewState->colorstyle = (ViewState->colorstyle + 1)%COLOR_COUNT;
    updateWidget(GUI_COLOR, (float)ViewState->colorstyle);
} 

void
toggleTerrainFly(void)
{
    ViewState->demoMode = !ViewState->demoMode;
    updateWidget(GUI_DEMO, (float)ViewState->demoMode);
}
...
#endif /* PERFLY */
#if 1 /* CUSTOM */
			case 'm':	/* Toggle terrain color */
				toggleColor();
			    break;

		    	case 'M':	/* Toggle terrain auto-fly */
			    toggleTerrainFly();
			    break;
