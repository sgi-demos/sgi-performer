...
#if 1 /* CUSTOM */
extern void advanceAnimations(void);
#endif /* CUSTOM */
...
#if 1 /* CUSTOM */
    initInset(ViewState->masterChan);
#endif
/* to indicate that the cliptexture center is to be set by masterChannel. this h as to be done after channels are initialized */
    {
...
#if 1 /* CUSTOM */
    ViewState->asdlist		= pfNewList(sizeof(pfASD*), 1, pfGetSharedArena());
    ViewState->insetWin		= PF_OFF;
    ViewState->preinsetWin 	= PF_ON;
    ViewState->loadRatio	= 0.5;
    ViewState->morphconstraint	= 8.0;
    ViewState->switchin		= 1.0;
    ViewState->preswitchin	= 1.0;
    ViewState->colorstyle	= PF_OFF;
    ViewState->precolorstyle	= PF_OFF;
    ViewState->fixpos		= PF_OFF;
    ViewState->moveratio	= 0.0;
    ViewState->morphattrs	= NULL;
    ViewState->origattrs 	= pfNewList(sizeof(ASDAttrs*), 1, pfGetSharedArena());
#endif /* CUSTOM */
...
initView(pfScene *scene)
{
    ...
    {
      /* Calculate near, far based on scene extent */
      ViewState->near = PF_MIN2(ViewState->sceneSize / 10.0f, 12.0f);
      ViewState->far = PF_MAX2(ViewState->sceneSize * 4.0f, 1000.0f);
      ViewState->nearFogRange = ViewState->near;
      ViewState->farFogRange = ViewState->far;
    }
#endif /* CUSTOM */
...
#endif /* PERFLY */
#if 1 /* CUSTOM */
	pfSetVec3(ViewState->viewCoord.hpr, 180.0f, 10.0f, 0.0f);
#endif /* CUSTOM */
...
#if 1 /* CUSTOM */
    ViewState->bsphere = bsphere;
    /* XXX We need a bbox here !!! */
...
#endif /* PERFLY */
#if 1 /* CUSTOM */
	    if(strsuffix(DatabaseFiles[i], ".config")) 
		root = loadASDConfigFile(DatabaseFiles[i]);
	    else
		root = pfdLoadFile(DatabaseFiles[i]);
#endif
...
#if 1 /* CUSTOM */
    pfuFindASDs((pfNode*)ViewState->sceneGroup, ViewState->asdlist);
    {
	pfASD *asd;
        int numasd = pfGetNum(ViewState->asdlist);
        int i;
        for(i = 0; i < numasd; i++)
        {
            asd = (pfASD *)pfGet(ViewState->asdlist, i);
	    if(asd != NULL)
	    {
		pfGetASDBBox(asd, &box);
		if(ViewState->lightPoints.addPoints)
		{
		    ViewState->lightGroup = pfNewGroup();
		    pfAddChild(ViewState->sceneGroup, ViewState->lightGroup);
		    flyLightsInit(asd);
		}
	    }
	}
...
updateChannel(void)
{
    ...
    if(ViewState->preinsetWin != ViewState->insetWin)
    {
	if(ViewState->insetWin == PF_ON) {
	    pfAddChan(pfGetChanPWin(ViewState->masterChan), ViewState->insetChannel);
	}
	else
	{
	    pfRemoveChan(pfGetChanPWin(ViewState->masterChan), ViewState->insetChannel);
	}
	ViewState->preinsetWin = ViewState->insetWin;
    }
}
#endif /* CUSTOM */
...
updateSim(pfChannel *chan)
{
...
#if 1 /* CUSTOM */
    static int cnt=0;
    float maxSpeed, angularVel, maxAccel;
    pfBox dbLimits;

    if(cnt == 2) {
        pfiGetXformerLimits(ViewState->xformer, &maxSpeed,
            &angularVel, &maxAccel, &dbLimits);
        pfiXformerLimits(ViewState->xformer, maxSpeed*0.8, angularVel*0.7,
            maxAccel, &dbLimits);
        cnt = 3;
    }
    if(cnt != 3) cnt++;
	...
	{
	    asd = (pfASD *)pfGet(ViewState->asdlist, i);
            /* Run Isect for all user selected segments. */
    	    if (ViewState-> isIsect)
    	    {
        	    pfHit **hits[32];

        	    (void) pfNodeIsectSegs(asd,
                                & ViewState -> segSet, hits);
    	    }

    	    /* Update the position of all objects moving on pfASD. */
    	    advanceAnimations();
	}
    }
#endif /* CUSTOM */
