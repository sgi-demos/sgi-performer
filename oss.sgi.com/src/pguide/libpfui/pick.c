...
main (int argc, char *argv[])
{
    ...
    pfNotify(PFNFY_NOTICE, PFNFY_PRINT, "Hit MIDDLE mouse to pick.");
    ...
    Shared->pick = pfiNewPick(arena);
    pfiPickHitFunc(Shared->pick, PickHitCB, NULL);
    Shared->inWindow = 0;
    Shared->reset = 0;
    Shared->exitFlag = 0;
    ...
    pfiAddPickChan(Shared->pick, chan);
    pfiSetupPickChans(Shared->pick);
	...
	UpdateView();
	if (Shared->mouseButtons & Button2Mask)
	{
	    int count = pfiDoPick(Shared->pick, Shared->mouseX, Shared->mouseY);
	}
	pfChanView(chan, Shared->view.xyz, Shared->view.hpr);
...
static int
PickHitCB(pfiPick *pick, void *data)
{
    int count = pfiGetPickNumHits(pick);
    if (count > 0)
    {
	pfNode *n = pfiGetPickNode(pick);
	if (n)
	    pfNotify(PFNFY_NOTICE,PFNFY_PRINT,
		"PickHitCB -- %d hits: first node %s of type %s", 
		count, pfGetNodeName(n), 
		pfGetTypeName(n));
	    else
		pfNotify(PFNFY_NOTICE,PFNFY_PRINT,
		"PICK %d hits: NULL pick node");
    }
    return PFTRAV_TERM; 
}
