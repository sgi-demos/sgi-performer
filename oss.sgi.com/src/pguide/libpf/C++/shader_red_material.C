...
main (int argc, char *argv[])
{
    ...
    geode->addGSet (pfdNewSphere (200, pfGetSharedArena ()));
    geode->setName ("geode1");

    root->addChild (geode);
    
    // create a shader
    pfShader *shader = new pfShader;
    
    // pass one sets material diffuse color to red
    shader->openPass (PF_SHADERPASS_GEOMETRY);
    pfGeoState *gstate2 = new pfGeoState;
    pfMaterial *mtl = new pfMaterial;
    mtl->setColor (PFMTL_DIFFUSE, 1, 0, 0);
    gstate2->setAttr (PFSTATE_FRONTMTL, mtl);
    gstate2->setAttr (PFSTATE_BACKMTL, mtl);
    shader->setPassAttr (PF_SHADERPASS_GSTATE, gstate2);
    shader->closePass ();

    shaderManager->applyShader (geode, shader);

    // traverse the scene graph looking for shaders
    ...
    double t;
    while ((t = pfGetTime() - startTime) < 4.0f)
    {
	pfCoord	   view;
	float      s, c;
