...
Usage (void)
{
    pfNotify(PFNFY_FATAL, PFNFY_USAGE, "Usage: buildarcinfo file.arcinfo tile_pre_name config_filename pagelookahead_filename\n");
    exit(1);
}
...
main(int argc, char *argv[])
{
    ...
    for (r=0, yval=yllcorner+(nrows-1)*cellsize;r<nrows;++r,yval-=cellsize)
        for (c=0, xval=xllcorner; c<ncols; ++c, xval+=cellsize)
        {
            if (fscanf(arcinfoFile, "%f", &zval) != 1)
                pfNotify(PFNFY_WARN, PFNFY_PRINT,
                   "buildarcinfo: Error reading data file");
    ...
    pfNotify(PFNFY_NOTICE, PFNFY_PRINT, "buildarcinfo: %s", argv[1]);
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "Dimensions: %d x %d vertices",
        ncols, nrows);

    lookahead[0] = lookahead[1] = 6;
    pfdBuildASD(ncols, nrows, tData, 0, 8, 1, argv[2], argv[3], argv[4], lookahead);
    pfFree(tData);
}
