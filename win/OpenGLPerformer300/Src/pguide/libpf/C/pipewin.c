 ...
 * pipewindow.c
 * 
 * OpenGL Performer pfPipeWindow example showing use of both
 * pure IRIS GL and X windows and the various pfPipeWindow features.
...
docmdline(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "fFbm:Rwp:?")) != -1)
    {
	switch (opt)
	{
	case 'f': 
...
main (int argc, char *argv[])
{
    ...
    Shared->exitFlag = 0;
    Shared->drawStats = 0;
    Shared->closeWin = 3; /* signals attach to window in GetXInput() */
    Shared->winSel = PFWIN_GFX_WIN;
    ...
    pfNotify(PFNFY_NOTICE,PFNFY_PRINT,"PipeWin Addr: 0x%p", Shared->pw);
    pfUserData(Shared->pw, (void*) 0xABABABAB);
    ...
    /* set off draw process to create window for X Input to attach to */
    pfFrame();
	...
	else if ((Shared->closeWin == 2) && (pfGetTime() - ct > 2.0f))
	{
	    pfConfigPWin(Shared->pw);
	    Shared->closeWin = 3;
	    pfNotify(PFNFY_NOTICE, PFNFY_PRINT, "OPEN");
	}
	else if ((Shared->closeWin == 0) && (Shared->winSel == PFWIN_STATS_WIN))
	{
	...
	else
	{
	    ...
	    pfFStatsClass(fstats, PFSTATSHW_ENGFXPIPE_FILL, PFSTATS_OFF);
	    pfDisableStatsHw(PFSTATSHW_ENGFXPIPE_FILL);
	}
	    
	if (!ForkedXInput)
	{
	    if (Shared->closeWin == 3)
		InitXInput(dsp);
	    GetXInput(dsp);
	}
    }
 ...
 *	placed in shared memory by sXPending().
 */
static void    
UpdateView(void)
{
...
pfNotify(PFNFY_NOTICE,PFNFY_PRINT,"In OpenPipeWindow 0x%p 0x%x", 
	pw, pfGetUserData(pw));
	...
	pfChooseWinFBConfig(over, OverlayAttrs);
    ...
    pfPrint(Shared->pw, 1,  PFPRINT_VB_ON, 0);
    pfuPrintPWinFBConfig(Shared->pw, 0);
    pfPrint(Shared->pw, 1,  PFPRINT_VB_ON, 0);
