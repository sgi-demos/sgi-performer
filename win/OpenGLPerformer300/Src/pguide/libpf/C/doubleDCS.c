 ...
 * or:  http://www.sgi.com
 *
 * doubleDCS.c : demonstrate a double-precision DCS node.
 *
 * $Revision: 1.3 $ $Date: 2002/11/14 00:02:43 $ 
 *
 */
...
Usage (void)
{
    pfNotify(PFNFY_FATAL, PFNFY_USAGE, "Usage: doubleDCS file.ext ...\n");
    exit(1);
}
...
main (int argc, char *argv[])
{
    ...
    /* Make a double-precision DCS node */

    double_dcs = pfNewDoubleDCS();

    /* Attach loaded file to a new pfScene. */
    scene = pfNewScene();

    pfAddChild(scene, double_dcs);
    pfAddChild(double_dcs, root);
	...
	pfSetVec3d(double_view.xyz, 2.0f * bsphere.radius * s, 
	...
	loadViewingMatrixOnDoubleDCS (double_dcs, &double_view);
    }
