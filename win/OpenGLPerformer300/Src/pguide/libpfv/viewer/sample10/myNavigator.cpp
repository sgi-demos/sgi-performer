...
pfType* myNavigator::classType = NULL;

///////////////////////////////////////////////////////////////////////////////

void myNavigator::init()
{
	if(classType==NULL)
	{
		pfObject::init();
		classType= new pfType(pfObject::getClassType(), "pfvWorld" );
	}
}
...
myNavigator::myNavigator(pfvXmlNode* root)
{
	...
	// bind input callback to keys 'r' and 'R', for resetting position 
	bindKeys("rR");
...
int myNavigator::handleEvent(int evType, char key)
{
	if( (key=='r') || (key=='R') )
	{
		printf("myNavigator (scope=%d,%d) resetting position\n",
				getScope(), getScopeIndex() );
		resetPosition();
		return 1;
	}
	return 0;
