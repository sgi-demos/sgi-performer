...
pfvmRotProjector::pfvmRotProjector()
{
...
void pfvmRotProjector::setPath( pfPath* _path )
{
	path->reset();
...
pfVec3 pfvmRotProjector::project(pfVec2&pt)
{
	if(chan)
	{
...
#define ROTATION_SCALAR 100.0f

pfVec3 pfvmRotProjector::projectAndGetRotation(pfVec2&pt,pfMatrix&rotmat)
{
    // Compute rot vec/angle
    pfVec3 axial( pt[0]-lastPt[0], pt[1]-lastPt[1], 0.0f );
    itre = axial.length()*ROTATION_SCALAR;
	...
	if( itre > 0.0f ) {
		...
		// hack: if <shift> is down, 'roll' object around view-direction

	    if( pfvInputMngr::getMouse()->modifiers & PFUDEV_MOD_SHIFT)
		{
			itre = axial[1]*ROTATION_SCALAR;
			//if(itre<0.0f)itre*=-1.0;
			qvec.set( 0.0f, 1.0f, 0.0f, 0.0f );
		}
		else
