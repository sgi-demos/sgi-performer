...
#ifdef WIN32
    iR = (_strnicmp(pfGetMachString(), "IR", 2) == 0);
#else
...
/* 
 * XXX Alex -- we need to figure out what's going on with
 * setting the videochannel id and then hard coding this won't
 * be necessary. But for now it will avoid us having to deal
 * with perfly crashing when fill stats are selected on win32.
 *
 */
#ifdef WIN32
    winIndex = 1;
