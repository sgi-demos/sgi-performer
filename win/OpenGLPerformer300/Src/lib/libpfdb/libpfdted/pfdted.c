...
pfdLoadFile_dted(char *fileName)
{
    FILE *dtedFile;
    ...
    if ((dtedFile = pfdOpenFile(fileName)) == NULL)
	return NULL;
    ...
    if (fread(&fileheader, sizeof(DTED_header_t), 1, dtedFile) != 1)
    {
	 pfNotify(PFNFY_WARN, PFNFY_PRINT,
	          "pfdLoadFile_dted: Error reading DTED file header");
        return(NULL);
    }
	...
	pfNotify(PFNFY_WARN, PFNFY_PRINT,
	   "pfdLoadFile_dted: Unsupported orientation angle.");
	...
	pfb_fread(inBuf, sizeof(short), num_rows, dtedFile);
    ...
    pfNotify(PFNFY_NOTICE, PFNFY_PRINT, "pfdLoadFile_dted: %s", fileName);
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "Dimensions: %d x %d vertices",
        num_cols, num_rows);
    node = pfdBuildASD(num_cols, num_rows, theData, 0, 6, 0, NULL, NULL, NULL, NULL);
