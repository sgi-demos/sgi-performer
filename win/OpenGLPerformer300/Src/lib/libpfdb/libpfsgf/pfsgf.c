...
/*
 * pfsgf.c: $Revision: 1.12 $ $Date: 2002/11/07 04:02:08 $
 */
...
/*
 * pfdLoadFile_sgf -- Load ".sgf" data files into IRIS Performer
 */

PFPFB_DLLEXPORT extern pfNode *
pfdLoadFile_sgf (char *fileName)
{
    FILE	*sgfFile	= NULL;
    pfNode	*node		= NULL;
    pfdGeom	*prim		= NULL;
    ...
    /* open ".sgf" file */
    if ((sgfFile = pfdOpenFile(fileName)) == NULL)
	return NULL;
    ...
    if (fgets(buffer, BUFFER_SIZE, sgfFile) == NULL)
    {
	fclose(sgfFile);
	return NULL;
    }
    ...
    /* read lines from ".sgf" file */
    while (fgets(buffer, BUFFER_SIZE, sgfFile) != NULL)
    {
	/* enlarge primitive if needed */
	if (numVerts >= maxVerts)
	    pfdResizeGeom(prim, maxVerts = 2*numVerts);

	/* read vertex data */
	pfSetVec3(prim->coords[numVerts], 0.0f, 0.0f, 0.0f);
	sscanf(buffer, "%lf,%lf,%lf", &x, &y, &z);

	/* is this a data vertex? */
	if (x < 1.0e36 || y < 1.0e36 || z < 1.0e36)
	{
	    prim->coords[numVerts][PF_X] = x;
	    prim->coords[numVerts][PF_Y] = y;
	    prim->coords[numVerts][PF_Z] = z;
	    totalVerts++;
	    numVerts++;
		...
		/* save just-used vertex */
		pfCopyVec3(prim->coords[0], prim->coords[1]);
		numVerts = 1;
	    }
	}
	else
	/* is's a terminating vertex */
	{
	    /* pick a random not-too-dark color */
	    pfuRandomColor(prim->colors[0], 0.4f, 0.8f);
    ...
    /* print statistics */
    pfNotify(PFNFY_NOTICE, PFNFY_PRINT, "pfdLoadFile_sgf: %s", fileName);
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "  Input Data:");
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "    Input vertices:     %8ld", totalVerts);
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "    Input groups:       %8ld", totalGroups);
