...
printCommands(void)
{
    ...
    fprintf(stderr, "  G      Print stats graph contents to stderr\n");
    fprintf(stderr, "  i      use immediate mode for pfGeoSets and pfScene\n");
    fprintf(stderr, "  h      Toggle scene graph\n");
...
void toggleVehicles(void)
{
	...
	if (ViewState->sceneGroup->removeChild(ViewState->vehicleDCSs[0]))
	    for (i = 1; i < ViewState->numVehicles; ++i)
		ViewState->sceneGroup->removeChild(ViewState->vehicleDCSs[i]);
	else
	    for (i = 0; i < ViewState->numVehicles; ++i)
		ViewState->sceneGroup->addChild(ViewState->vehicleDCSs[i]);
}
...
static int theresAClipTexture(void)
{
	...
	if (pfGetPipe(i)->getNumMPClipTextures() > 0)
	    return 1;
    return 0;
}
...
processKeybdInput(void)
{
	...
	if (events->devCount[dev] > 0)
	{
	    ...
	    {
	        pfCoord coord;
		ViewState->xformer->getCoord(&coord);
