...
#ifndef __PF_STATS_H__
#define __PF_STATS_H__
...
#include <Performer/pr/pfDispList.h>		// for DL_NUMCMDS
...
/*
 * Internal Stats Classes and Modes
 */

extern "C" {     // begin backdoor export to libpf
...
} // extern "C" end backdoor export to libpf
...
typedef struct pfStatsValFill	pfFillStats;
typedef struct pfStatsValIsect	pfIsectCounts;
typedef struct pfStatsValCalligraphic	pfCalligStats;

#ifdef USE_BTLNCK_STATS
typedef struct pfStatsValGfxPipeTimes	pfGfxPipeTimesStats;
#define _PFSTATS_NUM_BTLNCK_COUNTS (sizeof(pfGfxPipeTimesStats)/sizeof(float))
#endif // USE_BTLNCK_STATS

typedef struct _pfDispListStats {
    float		cmds[_PFDL_NUMCMDS];
} pfDispListStats;


typedef struct _pfStatsCounts {
    // start of structures of counts - all floats 
    pfGeomStats		geom;           // must be first 
    float		gstates, gstatesIndexed;

    // modeChanges must be the first of these 3 state things 
    pfModeStats		modeChanges; 
    pfModeStats		modeCalls;
    pfStateStats	state;
    pfXformStats	xform;
    pfFillStats		fill;
    pfCalligStats	callig;
    pfMemoryStats	memory;
    pfCPUStats		cpu;            // must be last of structures counts 
    pfStatsValTexLoads	texLoads;
} _pfStatsCounts;   



#define PFSTATS ((pfStats*)_pfCurrentBuffer->pf_indexUpdatable(this))

#define PFSTATSBUFFER ((pfStats*)buf->pf_indexUpdatable(this))

class DLLEXPORT pfStats : public pfObject 
{
public:
...
public:
    // sets and gets
    unsigned int	setClassMode(int _class, unsigned int _mask, int _val);
    unsigned int	getClassMode(int _class);
    void		setAttr(int _attr, float _val);
    float		getAttr(int _attr);

    unsigned int	setClass(unsigned int _enmask, int _val);
    unsigned int	getClass(unsigned int _enmask);

    // CAPI:verb GetStatsGfxPipeTimestampStatus
    int			getGfxPipeTimestampStatus(unsigned int stamp);
    // CAPI:verb GetStatsNumGfxPipeTimestamp
    int			getNumGfxPipeTimestamps();

    // CAPI:verb GetOpenStats
    unsigned int	getOpen(unsigned int _enmask);
	
public:
    // other functions
    // CAPI:verb
    unsigned int	open(unsigned int _enmask);
    static unsigned int	close(unsigned int _enmask);
    unsigned int	collectGfxPipe(unsigned int _enmask);
    void		reset();
    void		clear(unsigned int _which);
    void 		accumulate(pfStats* _src, unsigned int _which);
    void 		average(pfStats* _src, unsigned int _which, int _num);
    void		copy(const pfStats *_src, uint _which);
    // CAPI:verb StatsCountGSet
    void		count(pfGeoSet * _gset);

    int			query(unsigned int _which, void *_dst, int size);
    int			mQuery(unsigned int * _which, void *_dst, int size);

    // CAPI:noverb
    static void		setHwAttr(int _attr, float _val);
    static float	getHwAttr(int _attr);

    // CAPI:verb EnableStatsHw
    static void		enableHw(unsigned int _which);
    // CAPI:verb DisableStatsHw
    static void		disableHw(unsigned int _which);
    // CAPI:verb GetStatsHwEnable
    static unsigned int	getHwEnable(unsigned int _which);


PFINTERNAL: // XXX - suld make protected???
    int			index;
    unsigned int	statsopen;
    int			enable, haveCPUStats;
    unsigned int	mode[PFSTATS_NUM_CLASSES];  // mode settings 
    unsigned int	omode[PFSTATS_NUM_CLASSES]; // cache of opened modes 
    unsigned int	emode[PFSTATS_NUM_CLASSES]; // cache of opened modes 
    int			xo, yo, xs, ys; // window area corresponding to stats 
    int			dcBits;		// num of stencil bits for depth complexity 

    // start of structures of counts - all floats 
    _pfStatsCounts	counts;
    _pfStatsCounts	*min;
    _pfStatsCounts	*max;
    
    // these have to be handled carefully by Stats rtns 
    pfDispListStats	dlStats;
    pfCPUSysInfo	cpuSysInfo;
    pfGfxTimesData	gfxTimes;
#ifdef USE_BTLNCK_STATS
    pfGfxTStampBuf	tstampBuf;
    int			*glTStampBuf;
#endif
...
public:
    void *_pfStatsExtraData;
};

extern pfStats		*pfCurStats;
extern pfGeomStats	*pfCurGeomStats;
extern float		*pfCurTStripLengths;
extern _pfStatsCounts	*pfCurStateStats;
extern pfDispListStats	*pfCurDLStats;
extern pfCPUStats	*pfCurCPUStats;
extern pfStats		*pfCurGfxPipeStats;
extern pfFillStats	*pfCurFillStats;
extern int		*GLTStampBuffer;
extern pfStats		*pfCurTexLoadStats;
extern pfStats		*pfCurTexLoadBySizeStats;
extern pfStats		*pfCurCalligStats;

#ifdef USE_BTLNCK_STATS
extern pfGfxTStampBuf	*pfCurGfxTStampBuf;
#endif

#endif // !__PF_STATS_H__ 
