...
char **argv;
{
    ...
    zsize = image->zsize;
    if(zsize<3) {
	fprintf(stderr,"%s: this is not an RGB image file\n", argv[0]);
	exit(1);
    }
    for(y=0; y<ysize; y++) {
	...
	for (x=0; x<xsize; x++) {
	    /* use YIQ model to change to luminance (* 256 byte->short) */
	    color = 76.544f * rbuf[x] + 150.272f * gbuf[x] + 29.184f * bbuf[x];
	    cp[x] = color;
	}
	fwrite(packed, 2, xsize, output);
    }
    fclose(output);
    exit(0);
}
