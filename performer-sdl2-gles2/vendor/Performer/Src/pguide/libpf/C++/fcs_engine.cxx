...
static pfFCS *make_fcs(void)
{
    ...
    flux_rot = new pfFlux(sizeof(pfVec3), PFFLUX_DEFAULT_NUM_BUFFERS);
    flux_rot->setMode(PFFLUX_PUSH, PF_ON);

    matrix_engine = new pfEngine(PFENG_MATRIX);
    matrix_engine->setSrc(PFENG_MATRIX_ROT, flux_rot, NULL, 0, 0, 0);
    ...
    fcs->setMatType(PFMAT_ROT | PFMAT_TRANS);
    fcs->addChild(geode);
...
static void rot_tri(void)
{
    ...
    hpr = (pfVec3 *)flux_rot->getWritableData();

    hpr->vec[PF_H] = 0.0f;
    hpr->vec[PF_P] = 0.0f;
    hpr->vec[PF_R] = (float)count;

    flux_rot->writeComplete();
