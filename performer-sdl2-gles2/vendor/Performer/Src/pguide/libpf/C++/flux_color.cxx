...
//
//
// flux_color.C: example of a pfFlux being used by a pfGeoSet
//
// $Revision: 1.6 $ 
...
// local prototypes
//
static pfGeode *make_flux_color_geode(void);
static void warp_colors(void);
...
main (int argc, char *argv[])
{
    ...
    pfGeode *geode = make_flux_color_geode();
...
static pfGeode *make_flux_color_geode(void)
{
    ...
    flux_colors = new pfFlux(sizeof(pfVec4) * 3, PFFLUX_DEFAULT_NUM_BUFFERS);
...
static void warp_colors(void)
{
    ...
    c = count % 30;
    c1 = (c < 10)? c / 9.0f : ((c < 20)? (19-c) / 9.0f : 0.0f);
    c = (count + 10) % 30;
    c2 = (c < 10)? c / 9.0f : ((c < 20)? (19-c) / 9.0f : 0.0f);
    c = (count + 20) % 30;
    c3 = (c < 10)? c / 9.0f : ((c < 20)? (19-c) / 9.0f : 0.0f);

    colors = (pfVec4 *)flux_colors->getWritableData();

    PFSET_VEC4(colors[0], c1, c2, c3, 1.0f);
    PFSET_VEC4(colors[1], c2, c3, c1, 1.0f);
    PFSET_VEC4(colors[2], c3, c1, c2, 1.0f);
