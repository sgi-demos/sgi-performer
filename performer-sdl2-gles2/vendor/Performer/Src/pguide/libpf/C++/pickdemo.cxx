...
pfDCS* WhichModelHaveIPickedOn( pfNode* node )
{
	...
	/* go up to next DCS */
        ...
        while( node!=NULL && !((pfMemory*)node)->isExactType( pfDCS::getClassType() ) )
            node = ((pfGroup*)node)->getParent(0);
...
main (int argc, char *argv[])
{
    ...
    if( (models=(ModelData*)pfCalloc(numModels, sizeof(ModelData), 
	...
	models[i].root->getBound(&(models[i].bsphere));	
        width += (2.0f*models[i].bsphere.radius);      

        if((models[i].dcs = new pfDCS()) == NULL) 
        {
            pfNotify( PFNFY_FATAL, PFNFY_PRINT, "%s: new pfDCS failed.", argv[0] );
	    pfExit();
        }
	
	models[i].dcs->addChild( models[i].root );
	scene->addChild( models[i].dcs );
    }
    ...
    for( i=0; i<numModels; i++ )
    {
        models[i].dcs->setTrans( offset+models[i].bsphere.radius, 0.0f, 0.0f );
        offset +=  (2.0f*models[i].bsphere.radius);
        offset += GAPWIDTH;      
    }

    if( (hl = new pfHighlight) == NULL )
    {
        pfNotify( PFNFY_FATAL, PFNFY_PRINT, "%s: new pfHighlight failed", argv[0]);
        pfExit();		    
    }
	...
	pfFrame();
        ...
        /* pick on a model to select it, pick in the background to deselect */
        {
            ...
            if( mouse.click & PFUDEV_MOUSE_LEFT_DOWN )
            {
                ...
                if( chan->pick( (PFTRAV_IS_PRIM|PFPK_M_NEAREST), 
		    ...
		    picklist[0][0]->query( PFQHIT_NODE, &geode);
