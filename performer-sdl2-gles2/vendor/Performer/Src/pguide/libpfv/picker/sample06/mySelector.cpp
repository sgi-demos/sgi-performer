...
int mySelector::startHlite( pfvPicker*p, int prmsn )
{
...
int mySelector::updateHlite( pfvPicker* p,int ev,int prmsn, pfvPickerRequest*r )
{
	if((ev==LEFT_DOWN) || (ev==RIGHT_DOWN) )
	{
		r->state = PFPICKER_SELECT | PFPICKER_ALLOW_HLITE;
		r->selector = this;
	}
	return 1;
}
...
void mySelector::endHlite( pfvPicker* p )
{
