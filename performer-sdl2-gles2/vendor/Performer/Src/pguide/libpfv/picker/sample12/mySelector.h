...
public:
	...
	int select( pfNode* node );
	int deselect( pfNode* node );
	int toggle( pfNode* node );
	int deselectAll();
	int getNumSelected() { return listSel->getNum(); }
	pfNode* getSelected(int i) { return (pfNode*)(listSel->get(i)); }
			
	void printSel(char*prefix=NULL);
