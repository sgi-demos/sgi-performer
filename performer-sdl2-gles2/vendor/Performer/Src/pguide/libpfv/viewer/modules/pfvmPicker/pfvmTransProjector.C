...
#include "pfvmTransProjector.h"

///////////////////////////////////////////////////////////////////////////////

pfvmTransProjector::pfvmTransProjector()
{
...
void pfvmTransProjector::setPath( pfPath* _path )
{
	path->reset();
...
pfVec3 pfvmTransProjector::project(pfVec2&pt)
{
	if(chan && node)
	{
...
pfVec3 pfvmTransProjector::projectAndGetRotation(pfVec2&pt,pfMatrix&rotmat)
{
                                           ...
                                           int which )
{
    ...
    float xScale=chanNearPlaneDist*pfTan(fovH*0.5f);
    float yScale=chanNearPlaneDist*pfTan(fovV*0.5f);

    pfVec3 rangeV = (worldCenter - chanxyz);
    float range = rangeV.length();
    range /= chanNearPlaneDist;
    ...
    if( which ) 			
	axial.set( (pt[0]-startPt[0])*xScale, 
		   (pt[1]-startPt[1])*yScale, 0.0f);
    else
	axial.set( (pt[0]-startPt[0])*xScale, 
		   0.0f, (pt[1]-startPt[1])*yScale);
    
    axial.xformVec(axial, chanRot);		
    axial *= range;

    pfMatrix invPathMat;
    invPathMat.invertFull(pathMatrix);
    axial.xformVec(axial,invPathMat);
