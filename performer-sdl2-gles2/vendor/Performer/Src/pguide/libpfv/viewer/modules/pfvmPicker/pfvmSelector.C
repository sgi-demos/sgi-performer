...
{
public:
    UndoableSelAction( pfvmSelector*_sel, pfNode*_old, pfNode*_new ) {
...
{
public:
	UndoableDeleteAction( pfvmSelector*s, pfNode*_child ) {
		sel = s;
		child = _child;
    ...
    int undo() {
		...
		sel->select(child, 0);
		pfvmSelector::setClipboard(clip);
		return 1;
    }
	...
	int redo(){
		...
		sel->select(NULL, 0);
		clip=pfvmSelector::getClipboard();
		pfvmSelector::setClipboard(child);
		return 1;
    }
...
{
public:
	UndoablePasteAction( pfvmSelector*s, pfNode*_child ) {
		sel = s;
		child = _child;
		if(sel->node){
...
pfType* pfvmSelector::classType = NULL;
pfNode* pfvmSelector::clipboard = NULL;

///////////////////////////////////////////////////////////////////////////////

void pfvmSelector::init()
{
    if(classType==NULL)
    {
	pfvSelector::init();
	classType= new pfType(pfvSelector::getClassType(), "pfvmSelector" );
    }
}

///////////////////////////////////////////////////////////////////////////////

pfvmSelector::pfvmSelector(pfvXmlNode* xml)
{
    if(classType==NULL)
	    init();
    setType(classType);
    ...
    undoMngr = new pfvmUndoManager;
    PFASSERTALWAYS(undoMngr);

    dragger = new pfvmDragger(undoMngr);
    PFASSERTALWAYS(dragger);    
}

///////////////////////////////////////////////////////////////////////////////

pfvmSelector::~pfvmSelector()
{
    pfDelete(hl_sel);
}
...
int pfvmSelector::startSelection( pfvPicker*p, int ev )
{
    //printf("pfvmSelector::startSelection called\n");
    return 1;
}

///////////////////////////////////////////////////////////////////////////////

void pfvmSelector::resizeGeom(pfNode*_node)
{
    pfBox bbox;
	bbox.makeEmpty();
...
void pfvmSelector::select(pfNode*_node, int addToUndoList)
{
    //printf("pfvmSelector::select - %s %x\n",_node?_node->getTypeName():"NULL",_node );
    
    if(node==_node)
    {
		//printf("pfvmSelector::select - %x matches current selection\n",_node );
		return;
    }

    if(node) // deselect currently selected node
    {
		//printf("pfvmSelector::select - deselecting %x\n", node );
    ...
    if(_node)
    {
		...
		if( pfIsOfType(_node,pfScene::getClassType()) )
		{
	    	...
	    	//printf("pfvmSelector::select - creating a DCS to select all scene geom\n");
	    	pfDCS* _dcs = new pfDCS;
	    	PFASSERTALWAYS(_dcs);

	    	int i, numChildren=((pfGroup*)_node)->getNumChildren();
	    
	    	//printf("pfvmSelector::select - scene has %d children\n", numChildren );
	    	for(i=0;i<numChildren;i++)
	    	{
			    pfNode* child = ((pfGroup*)_node)->getChild(0);
			    if(child!=NULL)
				    _dcs->addChild(child);
			    //printf("pfvmSelector::select - removing child from scene\n");
		...
		{
			//printf("pfvmSelector::select - SELECTED node has no parent\n");
		...
		//printf("pfvmSelector::select - found parent %x\n", parent );
	
		if( (pfIsOfType(parent,pfDCS::getClassType())) &&
			(parent->getNumChildren()==1) )
		{
			//printf("pfvmSelector::select - parent is a DCS with 1 child\n");
			dcs = (pfDCS*)parent;
		...
		//printf("pfvmSelector::select - adding a DCS above node %x\n", _node );		
		pfDCS* _dcs = new pfDCS;
		PFASSERTALWAYS(_dcs);
    ...
    //printf("pfvmSelector::select - all is NULL\n");
...
pfNode* pfvmSelector::FIND_RECURSE_CHILD(pfGroup* parent, int childIndex)
{
...
pfNode* pfvmSelector::FIND_FIRST_CHILD( pfNode* _node, int childIndex )
{
...
pfGroup* pfvmSelector::FIND_PARENT( pfNode* child, int* childIndex )
{
	if(childIndex)
		*childIndex=-1;
	
	//printf("pfvmSelector::FIND_PARENT - root = %x child=%x\n", root, child );
...
pfNode* pfvmSelector::FIND_NEXT_SIBLING( pfNode* _node )
{
	int childIndex;
	pfNode* sibling;
...
pfNode* pfvmSelector::FIND_PREVIOUS_SIBLING( pfNode* _node )
{
	int childIndex;
	pfNode* sibling;
...
int pfvmSelector::specialFocus( pfvPicker* p,int ev,int prmsn,pfvPickerRequest*r )
{
	//printf("pfvmSelector::specialFocus - ev=%d (%c)\n", ev, ev );
	
	if(ev==LEFT_DOWN)
	{
		//printf("pfvmSelector::specialFocus - requesting selection\n");
		r->state = PFPICKER_SELECT | PFPICKER_FOCUS_EVENT;
		r->selector = this;

		if(!(state&PFPICKER_SELECT))
		{
			//printf("pfvmSelector::specialFocus - calling updateSelection\n");
...
int pfvmSelector::updateSelection( pfvPicker* p, int ev, pfvPickerRequest* r )
{
	
	//printf("pfvmSelector::updateSelection - node=%x\n", node );
	
	/*if(ev)
		printf("pfvmSelector::updateSelection -ev=%d (LEFT_DOWN=%d)\n", 
				ev, LEFT_DOWN );*/
	...
	{
		if(pfvmSelector::getClipboard())
		{
			undoMngr->add( new UndoablePasteAction(this,pfvmSelector::getClipboard()));
		}			
	}
	...
	if(ev=='c') /* copy selected geometry to clipboard */
	{
		if(node)
		{
			pfvmSelector::setClipboard(node);
...
void pfvmSelector::endSelection( pfvPicker* p )
{
    if(node)
    {
        select(NULL);
