...
void show_buttons(int start_row, float *analog, int dig);
...
main (int argc, char *argv[])
{
    ...
    flybox = pfuNewJoystick();
    pfuJoystickType(flybox, PFU_FLYBOX);
    pfuInitJoystick(flybox, NULL);

    js0    = pfuNewJoystick();
    pfuJoystickType(js0, PFU_JOYSTICK);
    pfuInitJoystick(js0, NULL);
...
/*
*/
	    pfuPrintJoystickAtRow(js0, 0);
            pfuGetJoystick(flybox, dummy_analog, &dummy_dig);
	    pfuPrintJoystickAtRow(flybox, 12);
...
"\033[2J"   - clear screen
*/
/* recommended start_row is 7 */
int THROTTLE_ROW    = start_row+1;
int THROTTLE_COLUMN = 8;
int BUTTON_ROW      = start_row+1;
int BUTTON_COLUMN   = 26;
int TRIGGER_ROW     = start_row+1+4;
int TRIGGER_COLUMN  = 45;
int HAT_ROW         = start_row+1;
int HAT_COLUMN      = 45;
int AXIS_ROW        = start_row+1;
int AXIS_COLUMN     = 60;
	int i, r, c;
