...
/*
 * pfstar.c: $Revision: 1.14 $ $Date: 2002/11/07 18:06:03 $
 */
...
/* length of longest input line in ".star" file */
...
/*
 * pfdLoadFile_star -- Load SGI ".star" astronomical files into IRIS Performer
 */

PFPFB_DLLEXPORT extern pfNode *
pfdLoadFile_star (char *fileName)
{
    FILE	*starFile;
    ...
    /* open ".star" file */
    if ((starFile = pfdOpenFile(fileName)) == NULL)
	return NULL;
    ...
    /* define a pfMaterial for the stars */
    m = (pfMaterial*)
	pfdGetTemplateObject(pfGetMtlClassType());
    pfMtlColor(m, PFMTL_AMBIENT,  0.0f, 0.0f, 0.0f);
    ...
    /* disable lighting (they provide their own ;-) */
    pfdBldrStateMode(PFSTATE_ENLIGHTING, PF_OFF);
    pfdBldrStateMode(PFSTATE_CULLFACE, PFCF_OFF);

    /* read ".star" file */
    while (fgets(buffer, BUFFER_SIZE, starFile) != NULL)
    {
	...
	if (SAME(token, "s"))
	{
	    /* read star definition from string */
	    sscanf(next, "%f %f %f %f", 
		&prim->coords[0][PF_X], 
		&prim->coords[0][PF_Y], 
		&prim->coords[0][PF_Z],
	    ...
	    /* people seem to like bright stars (change 0.8 to 0..1 at will) */
	    brightness = 1.0f + 0.8f*(brightness - 1.0f);

	    /* set point color */
	    pfSetVec4(prim->colors[0],
		brightness,
		brightness,
		brightness,
		1.0f
	...
	{
	    pfNotify(PFNFY_WARN, PFNFY_PRINT, "pfdLoadFile_star: unrecognized: %s", buffer);
	}
    }
    ...
    /* print statistics */
    pfNotify(PFNFY_NOTICE, PFNFY_PRINT, "pfdLoadFile_star: %s", fileName);
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "  Input Data:");
    pfNotify(PFNFY_INFO,   PFNFY_MORE,  "    Input stars:        %8ld", numStars);
