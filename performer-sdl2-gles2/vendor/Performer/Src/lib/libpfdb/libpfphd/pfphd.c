...
/*
 * pfphd.c: $Revision: 1.42 $ $Date: 2002/11/07 04:02:08 $
 */
...
/* length of longest input line in ".phd" file */
...
/*
 * pfdLoadFile_phd -- Load SGI ".phd" polyhedron files into IRIS Performer
 */

PFPFB_DLLEXPORT extern pfNode *
pfdLoadFile_phd (char *fileName)
{
    FILE	*phdFile;
    long	 seedValue;
    ...
    /* open ".phd" file */
    if ((phdFile = pfdOpenFile(fileName)) == NULL)
	return NULL;

    /* allocate primitive buffer */
    prim = pfdNewGeom(maxVerts);

    /* set color sequence to a random value */
    pfuRandomize(*(int *)&startTime);
    seedValue = pfuRandomLong();
    ...
    /* read ".phd" file */
    while (fgets(buffer, BUFFER_SIZE, phdFile) != NULL)
    {
	...
	/* identify token */
	if (SAME(token, "object.begin"))
	{
	    /* extract object name */
	    next = strchr(next, '"');
	    if (next != NULL)
	       next++;
	    strcpy(objectName, next);
	    next = strchr(objectName, '"');
	    if (next != NULL)
	       *next = '\0';
	}
	else
	if (SAME(token, "object.end"))
	{
	    ++numSkip;
	}
	else
	if (SAME(token, "vertex"))
	{
	    /* enlarge primitive buffer if necessary */
	    if (numVerts >= maxVerts)
	    {
		maxVerts *= 2;
		pfdResizeGeom(prim, maxVerts);
	    }

	    /* read coordinate from string */
	    sscanf(next, "%f %f %f", 
		&prim->coords[numVerts][PF_X], 
		&prim->coords[numVerts][PF_Y], 
		&prim->coords[numVerts][PF_Z]);

	    /* advance vertex count */
	    ++numVerts;
	}
	else
	if (SAME(token, "polygon.begin"))
	{
	    /* reset facet vertex count */
	    numVerts = 0;
	    ++numSkip;
	}
	else
	if (SAME(token, "polygon.end"))
	{
	    /* specify facet information */
	    ...
	    /* pick a random not-too-dark color based on vertex count */
	    pfuRandomize(seedValue + prim->numVerts);
	    pfuRandomColor(prim->colors[0], 0.2f, 0.7f);
	...
	{
	    pfNotify(PFNFY_WARN, PFNFY_PRINT, "pfdLoadFile_phd: unrecognized: %s", buffer);
	    ++numOther;
	}
    }
    ...
    if (node != NULL && objectName[0] != '\0')
	pfNodeName(node, objectName);
    ...
    if (numSkip != 0)
	pfNotify(PFNFY_INFO,   PFNFY_MORE,  
	    "    Commands skipped:   %8ld", numSkip);
    if (numOther != 0)
	pfNotify(PFNFY_INFO,   PFNFY_MORE,  
	    "    Commands unknown:   %8ld", numOther);
    if (totalVerts != 0)
	pfNotify(PFNFY_INFO,   PFNFY_MORE,  
	    "    Input vertices:     %8ld", totalVerts);
    if (totalTris != 0)
	pfNotify(PFNFY_INFO,   PFNFY_MORE,  
	    "    Input polygons:     %8ld", totalTris);
