...
/*
 * pfdoublescale.c - ".doublescale" pseudo-loader.
 *
 * To load a file and scale it, say:
 *	perfly <filename>.<scalefactor>.scale
 * (you won't notice any effect except in the sensitivity of the
 * controls in fly mode).
 * Also works with the ".rot" and ".trans" pseudo-loaders.
 *
...
pfdLoadFile_doublescale(char *fileName)
{
    double scale;
    char *lastdot, *firstdot;
    if ((lastdot = strrchr(fileName, '.')) == NULL
     || (firstdot = strrnchr(fileName, lastdot-fileName, '.')) == NULL
     || firstdot-fileName > 0 &&
        (firstdot[-1] == '.' || isdigit(firstdot[-1])) &&
        (firstdot = strrnchr(fileName, firstdot-fileName, '.')) == NULL
     || sscanf(firstdot+1, "%lf", &scale) != 1)
    {
	pfNotify(PFNFY_WARN, PFNFY_USAGE,
		"pfdLoadFile_scale: bad file name %s", fileName);
	return NULL;
    }
    ...
    pfNotify(PFNFY_INFO, PFNFY_PRINT, "Scaling scene by %g\n", scale);
