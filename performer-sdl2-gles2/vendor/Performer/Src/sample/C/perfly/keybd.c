...
extern pfCompositor *compositor;
...
toggleStats(void)
{
    ...
    ViewState->viewportOutline = 0;
    /* The GUI Controls this mode also, so update the Widget */
    updateWidget(GUI_STATS,(float)ViewState->stats);
}

void 
toggleViewportOutlines(void)
{
    ViewState->viewportOutline = !ViewState->viewportOutline;
    ViewState->stats = 0;
    /* The GUI Controls this mode also, so update the Widget */
    /* updateWidget(GUI_STATS,(float)ViewState->stats); */
}
...
void 
cycleCompositorMode(void)
{
     ViewState->compositorMode++;
     if(ViewState->compositorMode > 6)
         ViewState->compositorMode = 1;
     if(compositor != NULL)
     	pfCompositorMode(compositor, PFCOMP_TYPE, ViewState->compositorMode);
}
...
processKeybdInput(void)
{
	...
	if (events->devCount[dev] > 0)
	{
		...
		for (i = 0; i < events->numKeys; i++)
		{
			...
			case '1':	/* toggle FRUSTUM_CULL optimization */
				     	/* of CULL_SIDEKICK process         */
			    ViewState->optimizationMask ^= PFSK_FRUSTUM_CULL;
			    ViewState -> sidekickParamDirty = 1;
			    break;

			case '2':	/* toggle BACKFACE_CULL optimization */
				     	/* of CULL_SIDEKICK process         */
			    ViewState->optimizationMask ^= PFSK_BACKFACE_CULL;
			    ViewState -> sidekickParamDirty = 1;
			    break;

			case '3':	
			    switch (ViewState->sidekickPolicy)
			    {
				case PFSK_CULL_DONE:
				    ViewState->sidekickPolicy = 
						    PFSK_CULL_FRAME_DONE;
				    break;
				case PFSK_CULL_FRAME_DONE:
				    ViewState->sidekickPolicy = 
						    PFSK_SIDEKICK_DONE;
				    break;
				case PFSK_SIDEKICK_DONE:
				    ViewState->sidekickPolicy = 
						    PFSK_CULL_DONE;
				    break;
				default:
		...
		break;
	    case PFUDEV_F5KEY:		/* */
		toggleViewportOutlines();
		break;
	    case PFUDEV_F6KEY:		/* */
		cycleCompositorMode();
		break;
	    case PFUDEV_F7KEY:          /* */
                    pfCompositorMode(compositor, PFLOAD_BALANCE, !pfGetCompositorMode(compositor, PFLOAD_BALANCE));
		break;
