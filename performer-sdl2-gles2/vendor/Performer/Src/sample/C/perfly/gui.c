...
initGUI(pfChannel *masterChan)
{
    ...
    {
	pfuGUIViewport(a, a + 0.20 * (b-a), c, d);
	pfChanViewport(masterChan, a + 0.20 * (b-a), b, c, d);
	if(compositor)
	{
		pfCompositorViewport(compositor, a + 0.20 * (b-a), b, c, d);
	}
    } 
    else 
    {
	pfuGUIViewport(a, b, c, c + 0.20 * (d-c));
	pfChanViewport(masterChan, a , b, c + 0.20 * (d-c), d);
	if(compositor)
	{
		pfCompositorViewport(compositor, a , b, c + 0.20 * (d-c), d);
	}
    }
...
updateGUI(int guiOn)
{
	...
	if(ViewState->guiFormat == GUI_VERTICAL)
	{
	    ...
	    if(compositor)
		pfCompositorViewport(compositor, r, cb, cc, cd);
	}
	else
	{
	    pfChanViewport(ViewState->masterChan, ca, cb, t, cd);
	    if(compositor)
		pfCompositorViewport(compositor, ca, cb, t, cd);
	}
    }
    else    /* turn off GUI */
    {
	...
	if (Multipipe || (ViewState->numChans == 1)) 
	{
	    ...
	    if(compositor)
		pfCompositorViewport(compositor, 0.0f, 1.0f, 0.0f, 1.0f);
	}
	...
	else
	{
	    ...
	    if(compositor)
		pfCompositorViewport(compositor, ca, cb, cc, cd);
	}
    }
}
...
controlPanel(pfuWidget *w)
{
	...
	if (ViewState->haveMultisample || ViewState->haveFastStipple)
	{
	    if (ViewState->fade)
	    {
		pfChanLODAttr(ViewState->masterChan, PFLOD_FADE, ViewState->fadeRange);
...
/*     ViewState->num_widgets++; */
	...
	ViewState->guiState[ViewState->num_widgets].pane = -1;
	ViewState->guiState[ViewState->num_widgets].widget = GUI_LOD_FADE_RANGE;
	ViewState->guiState[ViewState->num_widgets].across = 1;
	ViewState->num_widgets++;
	if (!ViewState->fade)
	    pfuDisableWidget(wid);
